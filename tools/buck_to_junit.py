...
try:
  try:
    check_call(['curl', '--proxy-anyauth', '-sfo', path.join(temp_dir, 'saxon.jar'),
  ...
  buck_to_junit_xsl = abspath(path.join(path.abspath(path.dirname(__file__)), 'buckToJUnit.xsl'))

  chdir(args.o)
  try:
    check_call(['java', '-jar', path.join(temp_dir, 'saxon.jar'), '-s:' + buck_report, '-xsl:' + buck_to_junit_xsl])
  except CalledProcessError as err:
    exit(1)
finally:
