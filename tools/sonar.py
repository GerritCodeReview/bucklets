...
try:
  ...
  with ZipFile(path.join(plugin_dir, 'buck-out/gen/' + plugin_name + '.jar'), "r") as z:
    z.extractall(classes_dir)

  test_report = path.join(temp_dir, 'testReport.xml')
  call(['buck', 'test', '--no-results-cache', '--code-coverage', '--xml', test_report])

  junit_test_report_dir = path.join(temp_dir, 'junitTestReport')
  makedirs(junit_test_report_dir)

  try:
    check_call([path.join(path.abspath(path.dirname(__file__)), 'buck_to_junit.py'),
  ...
  generate_project_properties(plugin_name, plugin_dir, classes_dir, junit_test_report_dir, sonar_project_properties)

  try:
    check_call(['sonar-runner', '-Dproject.settings=' + sonar_project_properties, ])
  except CalledProcessError as err:
    exit(1)
finally:
